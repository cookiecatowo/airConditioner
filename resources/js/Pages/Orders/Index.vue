...
// 篩選狀態：從 URL 恢復，預設顯示 處理中+已完成，收款全部
const filterProcessing = ref(
    props.filters.ps ? props.filters.ps.split(',').map(Number) : [1, 2]
);
const filterPayment = ref(
    props.filters.pmt ? props.filters.pmt.split(',').map(Number) : [1, 2, 3]
);

const applyFilters = () => {
    router.get(route('orders.index'), {
        search: search.value || undefined,
        ps:  filterProcessing.value.join(','),
        pmt: filterPayment.value.join(','),
    }, { preserveState: true, replace: true });
};

watch(search, applyFilters);

const toggleFilter = (arr, value) => {
    const idx = arr.value.indexOf(value);
    if (idx === -1) {
        arr.value.push(value);
    } else {
        if (arr.value.length > 1) arr.value.splice(idx, 1); // 至少保留一個
    }
    applyFilters();
};

const getTypeBadgeClass = (type) => type === 'install'
    ? 'bg-blue-100 text-blue-800 border-blue-200'
    : 'bg-green-100 text-green-800 border-green-200';
const getTypeName = (type) => type === 'install' ? '安裝' : '維修';
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mb-6 flex flex-wrap gap-4">
                    <div class="flex items-center gap-2">
                        <span class="text-xs font-medium text-gray-500 whitespace-nowrap">處理狀況</span>
                        <div class="flex gap-1">
                            <button
                                v-for="o in processingOptions" :key="o.value"
                                @click="toggleFilter(filterProcessing, o.value)"
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-xs font-medium text-gray-500 whitespace-nowrap">收款狀況</span>
                        <div class="flex gap-1">
                            <button
                                v-for="o in paymentOptions" :key="o.value"
                                @click="toggleFilter(filterPayment, o.value)"
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="order in orders.data" :key="order.id" class="hover:bg-gray-50 transition" :class="{ 'opacity-50': order.processing_status == 3 }">
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap text-sm">
                                            <span :class="getTypeBadgeClass(order.type)" class="px-2 py-1 rounded-full text-xs font-bold border">
