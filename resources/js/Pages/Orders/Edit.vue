...
// 初始化資料轉換
const initialEquipments = props.order.equipments.map(e => ({
    id: e.id,
    brand_id: e.brand_id,
    brand_name: e.brand?.name || '',
    model_name: e.pivot.is_adjustment ? e.pivot.custom_model_name : e.model_name,
    specs: e.specs,
...
const initialMaterials = props.order.materials.map(m => ({
    id: m.id,
    name: m.pivot.is_adjustment ? m.pivot.custom_name : m.name,
    specs: m.specs,
    unit: m.unit || '組',
    unit_price: m.pivot.unit_price,
    quantity: m.pivot.quantity,
    item_note: m.pivot.item_note || '',
    is_adjustment: !!m.pivot.is_adjustment
}));

const form = useForm({
    customer_name: props.order.customer.name,
    customer_phone: props.order.customer.phone,
    customer_tax_id: props.order.tax_id || '',
    address: props.order.address,
    public_notes: props.order.public_notes,
    date: props.order.date,
    type: props.order.type,
    work_category: props.order.work_category || 'ac',
...
<template>
    <Head title="編輯報價單" />
    <AuthenticatedLayout>
        <template #header><h2 class="font-semibold text-xl text-gray-800 leading-tight">編輯報價單 #{{ props.order.id }}</h2></template>
        <div class="py-12"><div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <tbody>
                            <template v-if="form.type === 'install' && visibleEquipments.length > 0">
                                ...
                                <tr class="border border-black font-bold bg-gray-50/50">
                                    ...
                                    <td colspan="4" class="border border-black py-1">{{ equipmentsTotal }}</td>
                                </tr>
                            </template>
                            <template v-if="visibleMaterials.length > 0">
                                <tr v-for="(mat, i) in visibleMaterials" :key="'m'+i" class="border border-black">
                                    ...
                                    <template v-if="mat.is_adjustment"><td colspan="2" class="border border-black py-1">{{ mat.name }}</td><td colspan="3" class="border border-black py-1 font-bold">{{ mat.unit_price }}</td></template>
                                    <template v-else><td class="border border-black py-1">{{ mat.name }}</td><td class="border border-black py-1">{{ mat.specs }}</td><td class="border border-black py-1">{{ mat.quantity }} {{ mat.unit }}</td><td class="border border-black py-1">{{ mat.unit_price }}</td><td class="border border-black py-1">{{ mat.unit_price * mat.quantity }}</td></template>
                                    <td class="border border-black py-1">{{ mat.item_note }}</td>
                                </tr>
                                <tr class="border border-black font-bold bg-gray-50/50">
                                    <td colspan="2" class="border border-black py-1 text-center">小計</td>
                                    <td colspan="4" class="border border-black py-1">{{ materialsTotal }}</td>
                                </tr>
                            </template>
                            <tr class="border border-black font-black text-lg bg-blue-50/30">
                                ...
                                <td colspan="4" class="border border-black py-2 text-blue-700 font-bold">${{ totalAmount }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="bg-white p-6 shadow sm:rounded-lg flex justify-between items-center">
                    ...
                    <div class="text-right"><p class="text-sm text-gray-500">預估總金額</p><p class="text-3xl font-black text-blue-600">${{ totalAmount }}</p><PrimaryButton :disabled="form.processing" class="mt-4">儲存變更</PrimaryButton></div>
                </div>
            </form>
        </div></div>
    </AuthenticatedLayout>
